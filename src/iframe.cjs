  ...
  window.addEventListener("message", (event) => {
    if (event.data.type === "init-baby-monitor") {
      ...
      if (!(port instanceof MessagePort)) {
        console.error("Provided port is not a MessagePort instance.");
        return;
      }
      ...
      if (event.data.defaultPicker) {
        switch (event.data.defaultPicker) {
          ...
          default:
            console.warn(
              `Unknown defaultPicker '${event.data.defaultPicker}', defaulting to 'simple'`
            );
        }
      }
      ...
      if (event.data.pickerType) {
        switch (event.data.pickerType) {
          ...
          default:
            console.warn(
              `Unknown pickerType '${event.data.pickerType}', defaulting to 'num'`
            );
            window.useNumberOnlyPicker();
        }
      }
